...
* [`src/my_weather_app/styles.cljs`](src/my_weather_app/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/my_weather_app/`](src/my_weather_app/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/my_weather_app/core.cljs): contains the SPA entry point, `init`
* [`test/my_weather_app/`](test/my_weather_app/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `my-weather-app` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/my_weather_app/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns my-weather-app.example
  (:require [my-weather-app.config :as config])
